SUBROUTINE gov_seep(gveqnm, gveqne, i, j, k)
  ! ...   Purpose: To sum the conductance flow finite difference contributions
  ! ...        to the two governing equations for a given node.
  ! ...        For seepage cells to compute outflow rates
  USE machine_constants, ONLY: kdp
  USE bc, ONLY: ibc, seeping
  USE fdeq
  USE mesh
  USE parameters
  USE variables
  ...
  !CHARACTER(LEN=80) :: ident_string='$Revision: 1.5 $//$Date: 2007/08/17 19:07:26 $'
  !     ------------------------------------------------------------------
  gveqnm = 0._kdp
  gveqne = 0._kdp
  govm = 0._kdp
...
!!$           gveqne = gveqne + gove
     END IF
  END IF
  ! ...           skip if this node is at the top of the global mesh
  ! ...           or is below a non-active node
!!$  ! ...           or is below an active seeping node
  IF(k < nz) THEN
!!$     IF(ibc(ikzp,j) /= -1 .AND. .NOT.seeping(ikzp,j)) THEN
...
!!$           gveqne = gveqne + gove
     END IF
  END IF
  ! ...           skip if this node is at the bottom of the global mesh
  ! ...           or is above a non-active node
!!$  ! ...           or is above an active seeping node
  IF (k > 1) THEN
!!$     IF(ibc(ikzm,j) /= -1 .AND. .NOT.seeping(ikzm,j)) THEN
